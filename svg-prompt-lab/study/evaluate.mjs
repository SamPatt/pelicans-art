...
try {
 ...
 result.status='submitted';result.bytes=Buffer.byteLength(svg);result.sha256=createHash('sha256').update(svg).digest('hex');
 ...
 if(result.bytes>9000)result.warnings.push('Exceeded the requested 9000-character target (byte count shown separately)');
 ...
 try {
  ...
  const geometry=await page.evaluate(({svg,kind})=>{
   ...
   if(kind==='character'){
    ...
    for(const id of ['body','head-top','head-bottom'])if(get(id)?.parentElement!==root)errors.push(id+' must be a direct root group');
...
const report={studyId:study.id,run,execution:metadata,generatedAt:new Date().toISOString(),briefs:study.briefs,methods:[...study.methods,{id:'visual-revision',title:'One visual-feedback revision'}],results};
