...
## ARM64 installation and dialogue revision — 2026-09-06

Native GitHub Ubuntu 24.04 ARM64 rehearsal succeeded in [run 34042378085](https://github.com/SamPatt/pelicans-art/actions/runs/34042378085), runtime `e8261b2017d3e4fedb9f077a56c087527cf97715`. A fresh Python 3.12 environment installed all 47 hash-locked packages through the pip fallback, including Pocket 2.1.0 and CPU Torch 2.8.0. The same pinned April model/tokenizer and 26 preset embeddings were downloaded and prepared. Speech readiness passed in 4.3 seconds; three lines used Marius and Alba. Changing one line generated one new recording and reused two. Rendering then reused all three and produced 1280×720 H.264/AAC with 3/3 expected lines captured and muxed, and no capture diagnostics. Evidence is retained in the run's `pocket-arm64-evidence` artifact. This verifies a fresh native ARM Linux runner; the user's Hermes installation remains a separate manual test.

The rehearsal also found a Node subprocess timeout leak when optional `uv` was absent: preflight printed success but its process stayed alive until a 20-minute native spawn timeout expired. The CLI now owns and clears timeout timers on both errors and completion, with a regression test that requires a missing optional executable to exit promptly. Installer progress goes to stderr while stdout remains machine-readable JSON. Host tools, Python isolation checks, binary-wheel requirements, and model hashes remain enforced. Both architecture locks pin the same package versions; setup records the selected lock path and hash.
