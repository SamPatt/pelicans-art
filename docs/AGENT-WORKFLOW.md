...
Install FFmpeg using your OS package manager. `setup` installs npm dependencies and Chromium; Linux browser OS libraries may need `npx playwright install-deps chromium`. `setup --tts` adds an isolated pinned Pocket 2.1.0 environment under `.runtime/`, with CPU Torch 2.8.0 on Linux or Apple Silicon macOS. It does not alter the agent's environment or start services. Locked local Pocket setup supports Linux x64 and ARM64/WSL with glibc 2.28+, or Apple Silicon macOS, with Python 3.12. Intel Macs can render using supplied audio or an existing compatible speech endpoint.
...
## macOS rehearsal (2026-09-07)

[GitHub run 34127470318](https://github.com/SamPatt/pelicans-art/actions/runs/34127470318) used fresh macOS 15 Apple Silicon and Intel runners at runtime `555f0f751abe9ded43140f2444b69894ff595863`. Both passed all 10 CLI tests, including portrait rendering, supplied audio, preview, inspection, and packaging. On Apple Silicon, normal `setup --tts --python python3.12` installed hash-locked Pocket 2.1.0 and Torch 2.8.0 into the checkout's isolated environment, downloaded the pinned April model and preset files, and synthesized three lines. Changing one line regenerated one recording and reused two; the final render reused all three recordings and produced H.264/AAC video. The readiness probe succeeded after 6.29 seconds. CI artifacts retain the setup receipts, speech results, render manifest, and MP4.

Host prerequisites were provisioned on disposable runners (Node 22, Python 3.12, Homebrew FFmpeg). This verifies the CLI and CPU speech path, not interactive Safari, every macOS version, or a user's existing Homebrew environment. Intel local Pocket remains unavailable with the pinned Torch version. User installation still requires opt-in for missing host packages; setup itself does not install them.
