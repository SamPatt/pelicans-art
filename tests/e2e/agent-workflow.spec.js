...
test('agent instructions are discoverable, copyable, and phone-sized',async({page})=>{
 ...
 const download=await page.request.get('/downloads/pelican-theater-1.0.8.zip');expect(download.ok()).toBe(true);expect((await download.body()).subarray(0,2).toString()).toBe('PK');
});
