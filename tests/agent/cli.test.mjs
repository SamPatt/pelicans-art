...
test('finish delivers checked custom-output artifacts and detects archive corruption',async t=>{
 const root=await project(t),skit=await json(path.join(root,'skit.json'));skit.script=[{do:'pause',duration:0.3}];await writeJson(path.join(root,'skit.json'),skit);
 const result=await invoke(['finish',root,'--output',path.join(root,'custom output')]);assert.equal(result.code,0,result.error);assert.equal(result.ok,true);assert.equal(result.archiveVerified,true);assert.equal(result.requiresVisualReview,true);assert.equal(result.checks.pacing.advisory,true);assert.deepEqual(result.checks.findings,[]);assert.ok(result.phases.inspectSeconds>0);assert.ok((await fs.stat(result.contactSheet.path)).size>100);
