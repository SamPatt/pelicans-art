...
import {availableVoices} from './theater/voices.mjs';
...
const commandStartedAt=new Date().toISOString(),commandStarted=performance.now();
...
function parseOptions() {
  const commands = {
    ...
    voices:{endpoint:'string',select:'string'},
    assets:{match:'string',project:'string',source:'string',query:'string',category:'string',limit:'string','max-pages':'string',id:'string',name:'string',orientation:'string'},
    finish:{port:'string',output:'string'},preview:{serve:'boolean',port:'string','base-url':'string'},inspect:{'render-dir':'string'},package:{'render-dir':'string'}, help: {}, '--help': {}
  };
  ...
  const acceptsDirectory = (['svg','init','import','validate','build','render','finish','preview','inspect','package'].includes(command)||(command==='assets'&&assetAction==='add'));
...
const report = value => process.stdout.write(JSON.stringify({...value,execution:{startedAt:commandStartedAt,finishedAt:new Date().toISOString(),seconds:(performance.now()-commandStarted)/1000}},null,2)+'\n');
...
async function render() {
  ...
  try {
    ...
    return ({ok:true,...build,video:path.join(output,'project',manifest.media.video),cover:path.join(output,'project',manifest.media.cover),manifest:path.join(output,'project/manifest.json')});
  } finally {server.closeAllConnections();await new Promise(resolve=>server.close(resolve));}
}
try {
  parseOptions();
  if(flags.has('help')||!command||command==='help'||command==='--help')report({usage:'node scripts/theater.mjs <setup|doctor|voices|assets|svg|init|import|validate|build|preview|render|finish|inspect|package> [project-directory]',setup:'setup [--check] [--svg | --tts --python python3.12]: preflight then local npm/Chromium; locked Pocket requires Linux x64/ARM64 glibc 2.28+ or Apple Silicon macOS, with Python 3.12. Intel Macs can render with supplied audio or a speech endpoint. --svg installs only root npm/Chromium for standalone artwork, without FFmpeg/Python/TTS. --check makes no installation changes. Missing OS packages require explicit user opt-in.',doctor:'doctor [--endpoint http://127.0.0.1:8001/tts] [--wait SECONDS] [--json]',svg:'svg new-output-directory --source drawing.svg [--kind artwork|character|background|prop] [--model MODEL] [--title TITLE] [--description TEXT] [--tags TAGS] [--author NAME]: validate and deliver SVG + PNG preview; the agent authors the SVG directly.',init:'init path [--silent]',import:'import path --bundle /path/to/project.json',validate:'validate path',build:'build path',render:'render path [--port PORT] [--output PATH]',finish:'finish path [--port PORT] [--output PATH]: build, render, inspect and package; review the returned contact sheet before delivery.',voices:'voices [--endpoint URL] [--select alba,paul]: list verified pinned presets and subjective descriptions from the running Pocket service; missing selected IDs return exit code 1.',assets:'assets search --query TEXT [--match any|all] [--source all|local|pouch|project] [--project SOURCE_PROJECT] [--orientation portrait|landscape] [--category characters|props|backgrounds] [--limit N] [--max-pages N]; assets add project --source local|pouch|project [--project SOURCE_PROJECT] --category CATEGORY --id ID [--name NAME] [--orientation portrait|landscape]',preview:'preview project [--serve --port PORT | --base-url EXISTING_SERVER_ORIGIN]: build and capture a stage PNG; --serve keeps a loopback server alive until Ctrl-C; --base-url uses your already-running theater server.',inspect:'inspect project [--render-dir DIR]: inspect existing render; frames, contact sheet, decode and coverage checks. Does not render.',package:'package project [--render-dir DIR]: archive source, bundle and completed render; return MIME types, sizes and checksums. No upload.',output:'JSON on stdout; errors return exit code 1. No LLM provider calls.'});
  else if(command==='voices'){const result=await availableVoices(Object.fromEntries(flags));report(result);if(!result.ok)process.exitCode=1;}
  ...
  else if(command==='render')report(await render());
  else if(command==='finish'){
    const phases={};let start=performance.now();const rendered=await render();phases.buildAndRenderSeconds=(performance.now()-start)/1000;
    start=performance.now();const inspection=await inspectProject(directory,{renderDir:path.dirname(rendered.manifest)});phases.inspectSeconds=(performance.now()-start)/1000;
    if(!inspection.ok){report({ok:false,phase:'inspect',inspection,phases});process.exitCode=1;}
    else {start=performance.now();const delivery=await packageProject(directory,{renderDir:path.dirname(rendered.manifest)});phases.packageSeconds=(performance.now()-start)/1000;
      report({ok:true,phases,video:delivery.video,bundle:delivery.bundle,archive:delivery.archive,archiveVerified:delivery.archiveVerified,deliveryManifest:delivery.manifest,inspectionReport:inspection.report,contactSheet:inspection.contactSheet,checks:{duration:inspection.duration,orientation:inspection.orientation,streams:inspection.streams,dialogue:inspection.dialogue,findings:inspection.findings,pacing:inspection.pacing},requiresVisualReview:true,note:'Review the contact sheet before attaching these files. Listen where supported; automated checks do not judge acting or comedy.'});}
  }
