...
it('updates only missing speech, returns an editable bundle, and renders that bundle without new synthesis',async()=>{
 ...
 const wav=await run('ffmpeg',['-v','error','-f','lavfi','-i','sine=frequency=440:duration=2','-f','wav','pipe:1']);let requests=0;
 const server=createServer((req,res)=>{requests++;req.resume();res.end(wav);});await new Promise(r=>server.listen(0,'127.0.0.1',r));
 const config={engine:'pocket',endpoint:`http://127.0.0.1:${server.address().port}/tts`,textPrefix:''};const jobs=new EditorJobs({config:async()=>config});
 try{
  ...
  const edited=applyChanges(initial,[{op:'voice',character:'clerk',tempo:2,pitch:12,voice:'jean'}]);expect(Object.keys(edited.assets.audio)).toHaveLength(2);
  const created=await jobs.start(edited,'voices');await jobs.get(created.id).done;expect(jobs.get(created.id).message).toBeUndefined();expect(jobs.get(created.id).status).toBe('ready');expect(requests).toBe(4);
  const updated=await jobs.bundle(created.id);expect(updated.assets.audio['line-0']).toBe(initial.assets.audio['line-0']);expect(updated.assets.audio['line-2']).toBe(initial.assets.audio['line-2']);expect(prepareProject(updated).assets.audio).toEqual(updated.assets.audio);
