...
let project=null,selection={kind:'scene'},undo=[],redo=[],previewUrl,session=null,after=0,pollTimer,busy=false,proposal=null,proposalRevision,agentText='',agentNode=null,proposalAction=null,jobId=null,jobRunning=false,voicePresets=null,connectionState='disconnected';
...
function renderSelection(){
 ...
 if(selection.kind==='character'){
  ...
  const voiceLabel=el('label',{},'Voice preset'),voice=el('select',{'aria-label':'Voice preset'});for(const name of [...new Set([c.voice||'marius',...(voicePresets||[])])])voice.append(el('option',{value:name},name.replaceAll('_',' ')));voice.value=c.voice||'marius';voiceLabel.append(voice);box.append(voiceLabel);const tempo=inputField(box,'Voice tempo',c.voiceTempo??1,'number',{min:.5,max:2,step:.05}),pitch=inputField(box,'Voice pitch (semitones)',c.voicePitch??0,'number',{min:-12,max:12,step:.5});
  box.append(el('p',{class:'quiet'},'Choose a pinned preset. Tempo and pitch are independent. Saving marks this character’s recordings for update.'));
...
function chatBusy(value){busy=value;renderConnection();$('send-chat').disabled=value;$('cancel-chat').hidden=!value;}
async function disconnect(){clearTimeout(pollTimer);if(session)await api(`/sessions/${session}`,'DELETE').catch(()=>{});session=null;chatBusy(false);$('chat-form').hidden=true;$('connection-status').textContent='Disconnected. Your project is still saved here.';connectionState='disconnected';renderConnection();}
$('connect').onclick=async()=>{
 $('chat-panel').hidden=false;if(session||connectionState==='connecting')return;connectionState='connecting';renderConnection();$('connection-status').textContent='Checking your private agent connection…';$('connection-help').hidden=true;
 try{const status=await api('/status');voicePresets=status.voicePresets||voicePresets;if(project&&selection.kind==='character')renderSelection();if(!status.enabled){$('connection-help').hidden=false;$('connection-status').textContent='Connect from the Editor running alongside Hermes.';connectionState='disconnected';renderConnection();return;}const connection=await api('/sessions','POST',{});session=connection.id;after=0;connectionState='connected';renderConnection();$('connection-status').textContent='Connected to Hermes. This is a new project conversation using your existing Hermes setup.';$('chat-form').hidden=false;poll();}catch(e){$('connection-status').textContent=e.message;$('connection-help').hidden=false;connectionState='error';renderConnection();}
};
...
async function sendRequest(value){
 ...
 if(!project){chatMessage('Open a project first so Hermes has a scene to work with.');return false;}
 const request=value.trim();if(!request)return false;
 if(request.length>6000){notice('Keep your request under 6,000 characters.');return false;}
 const context=contextFor(project,selection);context.availableVoicePresets=voicePresets;
 if(selection.kind==='character'){const key=project.cast[selection.id].sprite+'-front';context.selectedSvg={name:key,svg:decode(project.assets.sprites[key])};}
 $('chat-panel').hidden=false;proposal=null;proposalAction=null;proposalRevision=project.editor.revision;$('apply-proposal').hidden=true;chatMessage(request,'user');agentText='';agentNode=chatMessage('');chatBusy(true);
 try{await api(`/sessions/${session}/messages`,'POST',{request,context});return true;}catch(e){chatMessage(e.message);chatBusy(false);return false;}
}
$('chat-form').onsubmit=async event=>{event.preventDefault();if(await sendRequest($('chat-input').value))$('chat-input').value='';};
$('send-selection').onclick=async()=>{if(await sendRequest($('agent-request').value))$('agent-request').value='';};
function renderConnection(){
 const connected=!!session;
 $('agent-status').textContent=connected?(busy?'Hermes connected · working…':'Hermes connected'):connectionState==='connecting'?'Connecting to Hermes…':connectionState==='error'?'Agent disconnected · reconnect':'Agent not connected';
 $('agent-status').dataset.connected=String(connected);
 $('connect').textContent=connected?'Open agent chat':connectionState==='connecting'?'Connecting…':'Connect your agent';$('connect').disabled=connectionState==='connecting';
 $('send-selection').hidden=!connected;$('send-selection').disabled=busy;$('copy-request').classList.toggle('secondary',connected);
 $('request-help').textContent=connected?'Send this note with the selected character or scene. Review the reply in agent chat.':'Paste into your agent chat with your latest exported project.';
}

async function poll(){if(!session)return;try{const result=await api(`/sessions/${session}/events?after=${after}`);for(const event of result.events){after=event.id;if(event.type==='text'){agentText+=event.text;if(!agentNode)agentNode=chatMessage('');agentNode.textContent=agentText;}else if(event.type==='permission')showPermission(event);else if(event.type==='error'){chatMessage(event.message);chatBusy(false);$('permission').hidden=true;if(event.fatal){session=null;connectionState='error';renderConnection();$('chat-form').hidden=true;$('connection-status').textContent='Connection ended. Reconnect to start a new conversation.';}}else if(event.type==='done'){chatBusy(false);$('permission').hidden=true;try{const block=agentText.match(/```(?:json)?\s*([\s\S]*?)```/);const parsed=JSON.parse(block?block[1]:agentText);if(Array.isArray(parsed.changes)&&(parsed.changes.length||['voices','render'].includes(parsed.action))){proposal=parsed.changes;proposalAction=['voices','render'].includes(parsed.action)?parsed.action:null;$('apply-proposal').hidden=false;}}catch{}if(!proposal)chatMessage('You can continue the conversation or copy a request to your usual agent chat.');}}}catch(e){chatMessage(e.message);session=null;connectionState='error';chatBusy(false);$('chat-form').hidden=true;$('connection-status').textContent='Connection ended. Reconnect to start a new conversation.';}if(session)pollTimer=setTimeout(poll,800);}
...
fetch('pocket-voices.json').then(r=>{if(!r.ok)throw Error('Voice list unavailable');return r.json();}).then(c=>{voicePresets??=c.voices;if(project&&selection.kind==='character')renderSelection();}).catch(()=>{});
