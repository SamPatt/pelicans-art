...
Repository: `https://github.com/SamPatt/pelicans-art.git`. Fetch it yourself into a fresh user-writable directory if no suitable checkout exists. For a downloaded release, check out the exact tested commit in the receipt's `ref` field and verify `git rev-parse HEAD` matches. For a skill read directly from a repository checkout, keep that checkout's revision. Do not reset or pull over user changes. Keep the runtime checkout and its `.runtime/` in a persistent user-writable directory for normal use; the temporary ZIP extraction directory is not the installation destination. Use a temporary runtime checkout only when the user requests a disposable test.

The repository is public. Agents can fetch `https://github.com/SamPatt/pelicans-art.git` over HTTPS without GitHub credentials, tokens, or a deploy key. If fetching fails, diagnose network, proxy, certificate, or sandbox restrictions; do not assume missing repository access. Do not request tokens in chat, generate SSH keys, change Git authentication or repository visibility, or require a manual clone as an installation shortcut.
...
The local Pocket installer supports Linux x64 and ARM64 (including compatible WSL installations) with glibc 2.28+, and Apple Silicon macOS, using Python 3.12. macOS 15 is the rehearsed Mac version. Intel Macs can render with supplied recordings or an existing compatible speech endpoint, but cannot install the pinned local Pocket profile because Torch 2.8.0 has no Intel Mac wheel. Do not silently substitute a different Torch version or remote speech provider. Native Windows is unsupported; use WSL. Rendering requires Node 22+, FFmpeg/ffprobe, and Playwright Chromium. Select an already installed Python 3.12 for local Pocket. Do not replace system Python or an agent's Python environment.

On a Mac, inspect the native architecture and installed tools first. Use native ARM64 Node and Python on Apple Silicon, rather than an Intel process under Rosetta. If host prerequisites are missing, explain the exact changes and obtain the user's opt-in as below. An existing Homebrew installation can provide missing `ffmpeg` and `python@3.12`; do not install or reconfigure Homebrew automatically. The CLI installs Chromium itself. No CUDA or Apple GPU configuration is needed: this Pocket profile runs on CPU.

Choose **one** setup path after inspecting existing speech services:

- For standalone SVG artwork, run `node scripts/theater.mjs setup --svg`. This installs only root npm dependencies and Chromium; it does not require FFmpeg, Python, server dependencies, or speech. Use the SVG command itself to verify the preview, not the full speech doctor.
...
Python packages and transitive dependencies are pinned with SHA-256 hashes in architecture-specific locks: `scripts/theater/pocket-linux-py312.lock` for x64 and `scripts/theater/pocket-linux-arm64-py312.lock` for Linux ARM64, and `scripts/theater/pocket-macos-arm64-py312.lock` for Apple Silicon. Preflight selects the lock automatically and records its relative path and SHA-256 in the installation receipt; installation requires binary wheels and checks hashes. Re-resolving or loosening the lock is a maintainer task, not an installation workaround.
