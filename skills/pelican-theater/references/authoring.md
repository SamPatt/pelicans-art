...
Optional in-editor Hermes chat runs only through the user's private theater server with `HERMES_EDITOR_ENABLED=1` and an already configured `hermes acp` executable. See the runtime's `docs/EDITOR.md` for configuration. It proposes undoable edits; new recordings and MP4 delivery still use this CLI workflow. The public Editor works without this connection.
