...
Run `node scripts/theater.mjs inspect <project>` and review its contact sheet and opening, reveal, and final frames. Run `node scripts/theater.mjs package <project>` for the standardized ZIP and checksums. See [workflow tools](workflow-tools.md). Check that captions leave the scene visible and the final beat is not cut off. Listening is preferable when supported. The capture process may take roughly the playback duration plus encoding; wait for completion rather than returning an unfinished path.
