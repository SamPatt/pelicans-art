...
Run `node scripts/theater.mjs finish <project>` for build/render, decoding and coverage checks, contact sheet, and an integrity-verified ZIP. Review the contact sheet and opening, reveal, and final frames once. Do not repeat successful automated checks with ad hoc scripts. See [workflow tools](workflow-tools.md). Check that captions leave the scene visible and the final beat is not cut off. Listening is preferable when supported. The capture process may take roughly the playback duration plus encoding; wait for completion rather than returning an unfinished path.
...
In DELIVERY-NOTES.md record the title, generation model, speech engine, runtime source/commit, what was checked, and how to import `output/project.json` or rerender the sources. Note that `project.json`'s speech endpoint may need changing on another machine. Record paths relative to the archive root. The package command checks referenced files and verifies the written ZIP directory and payload CRCs. If files are copied for delivery, compare the copy hash to the returned checksum; repeat media inspection only if the content changed. Packaging does not upload anything; use the current chat's delivery mechanism.
